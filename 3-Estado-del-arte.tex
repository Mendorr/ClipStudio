...
Para hablar de la edición de vídeo digital se deben tener claros algunos de sus elementos esenciales. Como el \textbf{píxel}, la unidad mínima de información visual de la que se compone una imagen. La disposición y cantidad de píxeles determinan la \textbf{resolución}, un factor que determina la calidad del vídeo. Entre las resoluciones más habituales se encuentran HD (1280×720), Full HD (1920×1080) y 4K (3840×2160), ampliamente utilizadas en distintos contextos audiovisuales según las necesidades de calidad y rendimiento (Jack, 2019)\cite{conceptos-basicos-3}. 

Otros conceptos fundamentales son la \textbf{relación de aspecto}, que define la proporción entre el ancho y el alto de la imagen, 16:9, estándar en televisión y plataformas digitales, y 21:9 en producciones cinematográficas. O la \textbf{fluidez del movimiento}, el número de fotogramas por segundo (FPS). Valores como 24 FPS se asocian tradicionalmente al cine, mientras que 30 o 60 FPS son habituales en contenidos televisivos y digitales, especialmente en videojuegos o retransmisiones en directo. Normalmente, cuanto mayor sea la resolución y tasa de fotogramas de un vídeo, mayor será su calidad, así como su tamaño (véase Figura~\ref{fig:resoluciones-comunes}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{assets/resoluciones-comunes.png}
    \caption[Resoluciones Comunes]{Resoluciones Comunes\\ \footnotesize Fuente: \cite{resoluciones-comunes}}
    \label{fig:resoluciones-comunes}
\end{figure}

 En esta figura, se pueden observar las resoluciones más comunes. Desde 360p que en su día era considerado como HD (High Definition) hasta resoluciones más modernas como el 4K o el 8k, las cuales no son visibles en todas las pantallas.

\subsection{Formatos de vídeo}

Un aspecto técnico a tener en cuenta es la gestión de formatos. Lo que 
coloquialmente se denomina ``archivo de vídeo'' es una estructura compleja que agrupa 
diversos tipos de datos en un solo bloque. Es necesario comprender y diferenciar los conceptos relacionados con: 
\textbf{contenedor} (o encapsulamiento) y \textbf{códec}.

Esta división es a menudo malinterpretada, asumiendo que la extensión del archivo 
(por ejemplo, .mp4 o .avi) define por completo su formato y calidad. En realidad, la extensión solo 
identifica al contenedor, mientras que los datos audiovisuales que alberga en su interior están regidos 
por sus respectivos códecs.

\subsubsection{Los Códecs}

La palabra ``códec'' es un acrónimo de los términos ``compresor/descompresor'' 
(o codificador/decodificador). Un archivo de vídeo, contiene una cantidad masiva de datos 
(imágenes, audio, metadatos, entre otros). El trabajo del códec es codificar y comprimir estos datos originados por 
la cámara para reducir el espacio de almacenamiento necesario, minimizando colores, reduciendo resoluciones 
o fusionando datos similares. Posteriormente, durante la reproducción o edición, el mismo algoritmo decodifica 
y descomprime la información (véase Figura~\ref{fig:codec}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{assets/codec.jpeg}
    \caption[Códec]{Funcionamiento general Códec\\ \footnotesize Fuente: \cite{funcionamiento-codec}}
    \label{fig:codec}
\end{figure}

Los códecs presentan diferentes métodos de compresión:

\begin{itemize}
    \item \textbf{Con pérdida (Lossy):} El algoritmo elimina o fusiona información de 
    forma irreversible para reducir drásticamente el peso del archivo. La calidad disminuye progresivamente 
    con cada renderizado o edición posterior.
    \item \textbf{Sin pérdida (Lossless):} Almacena los datos conservando las propiedades 
    del archivo original. Genera una calidad superior en función de archivos con un tamaño y peso 
    considerablemente mayores.
\end{itemize}

Entre los códecs más relevantes en la industria actual destacan~\cite{tipo-formato-arch}:

\begin{itemize}
    \item \textbf{H.264:} Es el estándar en la actualidad. Permite una calidad de Alta Definición 
    (HD) e incluso 4K, ofreciendo una multitud de opciones de compresión. Es el códec más recomendado para 
    plataformas de distribución masiva de archivos multimedia, como YouTube.
    \item \textbf{H.265 (HEVC - High Efficiency Video Coding):} Diseñado como sucesor 
    del H.264. Su principal ventaja es la optimización del ratio peso/calidad, 
    aspecto fundamental para la distribución de contenido en entornos digitales saturados 
    (p.ej, Redes Sociales y Plataformas de contenido, entre otros).
    \item \textbf{Apple ProRes:} Formato en propiedad de Apple con prestaciones de 
    compresión similares al H.264, pero orientado a la edición profesional. Ofrece soporte para 
    transparencias (canal alfa) en ciertas variaciones. Históricamente requería decodificadores 
    específicos en entornos PC/Linux, aunque su adopción se ha estandarizado en la alta producción.
    \item \textbf{DivX / XviD:} Códecs clásicos que popularizaron la distribución 
    de vídeo en resoluciones estándar a través de internet. XviD se presenta como la alternativa de 
    código abierto a DivX.
\end{itemize}

\subsubsection{Contenedores de Vídeo}

El contenedor es el ``recipiente'' que reúne y sincroniza todos los elementos codificados 
(vídeo, audio, subtítulos, menús y metadatos) para su reproducción simultánea. Suelen identificarse por la 
extensión del archivo. A continuación, se detallan los contenedores más relevantes del sector, con sus ventajas y 
limitaciones~\cite{v-d-formato-video}:

\begin{itemize}
    \item \textbf{MP4 (.mp4):} Es el formato más extendido, versátil y popular. 
    Compatible con prácticamente cualquier dispositivo, sistema operativo y plataforma web. 
    Ofrece una alta calidad con tamaños de archivo reducidos.
    \item \textbf{MOV (.mov):} Desarrollado por Apple (QuickTime), es un estándar en la edición 
    profesional. Soporta pistas divididas, transparencias y efectos. Su principal desventaja es generar 
    archivos muy pesados y tener compatibilidad limitada fuera del ecosistema Apple.
    \item \textbf{MKV (.mkv):} Contenedor de código abierto en tendencia. Su gran fortaleza 
    es la capacidad de albergar múltiples códecs simultáneamente, numerosas pistas de audio, 
    subtítulos y menús sin pérdida de datos en la compresión. En cambio, genera archivos de gran tamaño.
    \item \textbf{WEBM (.webm):} Formato de código abierto desarrollado por Google,
    optimizado específicamente para HTML5. Se reproduce nativamente en navegadores sin necesidad de \textit{plugins}, 
    ofreciendo tiempos de carga mínimos.
    \item \textbf{AVI (.avi):} Pionero desarrollado por Microsoft en 1995. 
    Ofrece excelente calidad audiovisual, pero está obsoleto para los estándares actuales de streaming 
    debido a sus enormes tamaños de archivo y la incapacidad de almacenar subtítulos de forma nativa.
    \item \textbf{WMV (.wmv):} Sucesor del AVI, supera al MP4 en capacidad de compresión y es 
    ideal para la venta de vídeo digital gracias a su soporte para licencias (DRM~\cite{drm}). Sin embargo, tiene 
    baja aceptación en reproductores.
    \item \textbf{AVCHD:} Formato de vídeo con alta definición, creado por Sony y Panasonic 
    para videocámaras. Utiliza compresión H.264/MPEG-4 para lograr tamaños reducidos sin pérdida de datos, 
    pero tiene una compatibilidad muy limitada sobre dispositivos reproductores.
\end{itemize}

\subsubsection{Selección de formato}

La elección del formato adecuado en un flujo de trabajo depende del objetivo que se quiera 
abordar en el proyecto. Para la distribución digital masiva, las plataformas han establecido directrices 
claras para maximizar la eficiencia (véase el Cuadro~\ref{tab:eleccion-formato})~\cite{v-d-formato-video}
...
\hline
\textbf{Plataforma} & \textbf{Contenedor Rec.} & \textbf{Códec de Vídeo} & \textbf{Notas de Optimización} \\ \hline
YouTube & MP4 & H.264 & Usar con códec de audio AAC-LC. \\ \hline
Instagram & MP4 (soporta MOV) & H.264 & MP4 minimiza la pérdida de datos y tiempos de subida. \\ \hline
Facebook & MP4 (soporta MOV) & H.264 & Prioriza MP4 por equilibrio peso/calidad. \\ \hline
TikTok & MP4 & H.264 & Estándar nativo de la aplicación. \\ \hline
Navegadores Web & WEBM o MP4 & VP8/VP9 H.264 & WEBM permite incrustación directa en HTML5. \\ \hline
\end{tabular}
\caption{Recomendaciones de formato}
\label{tab:eleccion-formato}
\end{table}
\vspace{0.5cm}


La multitud de opciones obliga a los creadores a hacer más que simplemente elegir 
una extensión. Deben diseñar un flujo de compresión donde el contenedor y el códec trabajen en sinergia 
para satisfacer las demandas técnicas de la plataforma de destino.



 \subsection{Postproducción}
 Por otro lado, la \textbf{postproducción} es una de las fases más determinantes en la creación y edición de vídeo, ya que en ella se da forma definitiva al material audiovisual grabado. Las posibilidades de la edición son infinitas y van únicamente sujetas a la creatividad y conocimientos del editor. Uno de los principales objetivos es conseguir obtener una narrativa mezclando las distintas vistas (Dancyger, 2014)\cite{conceptos-basicos-2}. La postproducción incluye también la incorporación de efectos visuales (VFX), como transiciones, composiciones digitales, títulos o animaciones, que amplían las posibilidades creativas del medio y facilitan la integración de elementos gráficos en el vídeo final (Watkinson, 2018)\cite{conceptos-basicos-4}.

 Pero antes de profundizar en este mundo, es necesario quedar claros algunos conceptos básicos. Una serie de operaciones básicas que permiten organizar, estructurar y perfeccionar el material grabado hasta obtener una pieza coherente y finalizada. Entre las acciones más elementales se encuentran cortar y dividir clips, lo que permite eliminar fragmentos innecesarios y ajustar la duración de cada plano para mejorar el ritmo narrativo. Del mismo modo, la operación de copiar y pegar facilita la reorganización del material dentro de la línea de tiempo, permitiendo modificar el orden de las escenas o reutilizar fragmentos concretos. Estos procesos se realizan dentro de un entorno de edición no lineal (NLE) (véase Figura~\ref{fig:editor-de-video}), donde el contenido se organiza en pistas de vídeo y pistas de audio independientes.
 \begin{figure} [H]
    \centering
    \includegraphics[width=1.0\linewidth]{assets/editor-video.png}
    \caption[Editor]{Editor de vídeo\\ \footnotesize Fuente: \cite{editor-video}}
    \label{fig:editor-de-video}
\end{figure}
 
\subsubsection{Pistas de vídeo}
...
\begin{figure}[H]
     \centering
     \includegraphics[width=0.75\linewidth]{assets/etalonaje-y-correcion-de-color-blog-ejemplos.jpg}
     \caption[Etalonaje]{Etalonaje del color\\ \footnotesize Fuente: La La Land (2016)}
     \label{fig:etalonaje}
 \end{figure}

 La corrección de color es el procedimiento de ajustar la reproducción de los colores en una imagen o vídeo para que sean precisos y coherentes con un estándar o referencia concreta (véase Figura~\ref{fig:la-la-land}).
 Este proceso se realiza para lograr una continuidad visual. Por ejemplo, cuando se filma una escena, cada plano puede ser grabado en diferentes momentos del día, con diferentes condiciones de iluminación y enfoque, lo que puede hacer que los colores parezcan diferentes de un plano a otro.(Lourdes Millán - Animun3D)\cite{chromakey}.
   \begin{figure}[H]
     \centering
     \includegraphics[width=1.0\linewidth]{assets/correcion-color.png}
     \caption[Correccion Color]{Corrección del color\\ \footnotesize Fuente: La La Land (2016)}
     \label{fig:la-la-land}
 \end{figure}
...
\subsubsection{Efectos visuales VFX}
...
Uno de los más influyentes actualmente es el trabajo de \textbf{Motamed, Wouter y Luck}~\cite{Motamed_2024_CVPR}, analizando el papel 
...
El atrículo científico parte de la idea de que, en modelos texto-a-imagen, la atención cruzada desempeña 
...
, concluyendo el estudio en que la atención cruzada 
consitituye un mecanismo clave, habilitando la capacidad de un \textbf{modelo texto-a-video} para editar texto, imágenes 
o vídeos siguiendo instrucciones (\textit{prompts}) directos, sin haber sido entrenado específicamente 
...
de vídeo mediante difusión \cite{Motamed_2024_CVPR}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{assets/atencion-cruzada.png}
    \caption[Representacion cruzada]{Representación visual del concepto \textbf{atención cruzada}\\ \footnotesize Fuente: \cite{Motamed_2024_CVPR}}
...
\begin{figure}[H]
    ...
    \caption{[Orientacion pasada]Representación visual del concepto \textbf{orientación pasada}\\ \footnotesize Fuente: \cite{Motamed_2024_CVPR}}
